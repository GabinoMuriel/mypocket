...
export function UserView({ userData, onClose, onSuccess }: AdminUserPanelProps) {
    ...
    return (
        <Sheet open={open} onOpenChange={handleOpenChange}>
            <SheetContent className="sm:max-w-[450px] overflow-y-auto flex flex-col p-3">
                <SheetHeader>
                    <SheetTitle className="text-2xl font-bold">Detalles del Usuario</SheetTitle>
                </SheetHeader>

                {/* Read-only Data */}
                <div className="mt-6 space-y-2 text-sm text-muted-foreground mb-6 border-b pb-4">
                    <p><strong>Email:</strong> {userData.email}</p>
                    <p><strong>Rol:</strong> {userData.role_name}</p>
                    <p><strong>ID:</strong> <span className="text-xs">{userData.id}</span></p>
                    <p><strong>Fecha de Registro:</strong> <span className="text-xs">{userData.created_at}</span></p>
                </div>

                {/* Editable Form */}
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 flex-1">
                    <FormInput label="Nombre" type="text" error={errors.first_name?.message} {...register("first_name")} />
                    <FormInput label="Apellidos" type="text" error={errors.last_name?.message} {...register("last_name")} />
                    <FormInput label="Teléfono" type="text" error={errors.phone?.message} {...register("phone")} />
                    <FormInput label="Fechade de nacimiento" type="date" error={errors.birthdate?.message} {...register("birthdate")} />
                ...
                <div className="mt-8 pt-6 border-t border-destructive/20">
                    <h4 className="text-sm font-semibold text-destructive mb-4">Zona de Peligro</h4>
                    <DeleteUserDialog
                        userId={userData.id}
